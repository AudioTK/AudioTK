...
TEST(TS9OverdriveFilter, sinus_drive_test)
{
  ATK::TS9OverdriveFilter<double> filter;
  filter.set_drive(0.5);
  ASSERT_EQ(filter.get_drive(), 0.5);
}

TEST(TS9OverdriveFilter, sinus_drive_range_test)
{
  ATK::TS9OverdriveFilter<double> filter;
  ASSERT_THROW(filter.set_drive(1.0000001), std::out_of_range);
}

TEST(TS9OverdriveFilter, sinus_drive_range2_test)
{
  ATK::TS9OverdriveFilter<double> filter;
  ASSERT_THROW(filter.set_drive(-0.0000001), std::out_of_range);
}

TEST(TS9OverdriveFilter, const_sin1k)
{
  ...
  {
    std::ifstream check(ATK_SOURCE_TREE "/tests/data/output4_ts9.dat", std::ios::binary);
